    ...
    try {
      ...
      if (statusResult.success) {
        if (statusResult.status === 'payment_completed' || statusResult.status === 'escrowed') {
          ...
          const response = await api.get(endpoints.transactions.detail(transactionId!));
      ...
      // Assume success for demo if backend fails or timeout
      setVerified(true);
      toast.warning('Payment status pending, check dashboard shortly.');
    } finally {
      setVerifying(false);
    }
